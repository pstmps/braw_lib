...
def get_framework_path() -> str:
    """
    Find out if the application is running as a bundle (packaged by PyInstaller)
    or as a standalone Python script, and return the path to the `BlackmagicRawAPI.framework`.
    Returns:
        The path to the `BlackmagicRawAPI.framework`.
    """
    if getattr(sys, 'frozen', False):
        return sys._MEIPASS
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Libraries')

def read_metadata(filename: str, binaryDirectory: str = None) -> dict:
    """
    ...
    if binaryDirectory is None:
        binaryDirectory = get_framework_path()

    metadata_raw = _bmd_metadata.read_metadata(filename, binaryDirectory)
    metadata = {}
    for key, value in metadata_raw.items():
        metadata[key] = _convert_str(value.encode('utf-8', 'ignore').decode('utf-8').strip())
    return metadata

def _convert_str(s: str) -> (int, float, str):
    """
    Convert a string to an int, float, or str.
    Args:
